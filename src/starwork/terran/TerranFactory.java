...
public class TerranFactory {
	...
	private static UnitType TANK_TANK_MODE = UnitType.Terran_Siege_Tank_Tank_Mode;
	public static UnitType VULTURE = UnitType.Terran_Vulture;
	public static UnitType GOLIATH = UnitType.Terran_Goliath;
	...
	private static final UnitType buildingType = UnitType.Terran_Factory;
	...
	public static boolean shouldBuild() {
		...
		if (TerranBunker.GLOBAL_MAX_BUNKERS >= 2 && TimePresident.getTimeSeconds() < 500) {
			if (!xvr.canAfford(250) && TerranBunker.getNumberOfUnits() < TerranBunker.MAX_STACK) {
				return ShouldBuildCache.cacheShouldBuildInfo(buildingType, false);
			}
		}

		if (factories >= 1 && !xvr.canAfford(550) && TimePresident.getTimeSeconds() < 800
	...
	private static UnitType defineUnitToBuild(int freeMinerals, int freeGas) {
		...
		boolean notEnoughGoliaths = TimePresident.getTimeSeconds() < 800 ? goliaths < MINIMUM_GOLIATHS_EARLY
