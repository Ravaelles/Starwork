...
public class UnitDamages {
	...
	public static void rememberUnitDamageValues() {
		for (UnitType unitType : UnitType.getAllTypes()) {
			// System.out.println(unitType);
			if (!unitType.c_str().startsWith("Terran") && !unitType.c_str().startsWith("Protoss")
					&& !unitType.c_str().startsWith("Zerg")) {
				continue;
			}

			try {
				int groundAttackValue = unitType.getGroundWeapon().damageAmount();

				if (unitType == UnitType.Protoss_Zealot || unitType == UnitType.Terran_Firebat) {
					groundAttackValue *= 2;
				}
				...
				if (groundAttackValue == 0) {
					...
					double groundAttackValueNormalized = ((double) 30 * groundAttackValue / unitType.getGroundWeapon()
							.damageCooldown());
					groundAttackValueNormalized = Double.parseDouble(String.format("%.1f", groundAttackValueNormalized,
							Locale.ENGLISH).replace(',', '.'));
					groundAttackValuesPerSec.put(unitType, groundAttackValueNormalized);
				}
	...
	public static void displayUnitDamages() {
		...
		Map<UnitType, Integer> sorted = RUtilities.sortByValue(groundAttackValues, false);
		...
		Map<UnitType, Double> sorted2 = RUtilities.sortByValue(groundAttackValuesPerSec, false);
		...
		for (UnitType type : sorted2.keySet()) {
			...
			double value = sorted2.get(type) < 1 ? 0 : (1000 * sorted2.get(type) / cost);
			pricesMap.put(type, value);
		}

		Map<UnitType, Double> sorted3 = RUtilities.sortByValue(pricesMap, false);
